...
export default function AdminSchedule() {
  ...
  const exportToCSV = () => {
    ...
    link.download = `org_schedule_${monthName}_${year}.csv`;
    link.click();
  };
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white p-4 md:p-6 space-y-6">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
          <h2 className="text-3xl md:text-4xl font-black italic tracking-tighter uppercase leading-none">
            Event <span className="text-orange-600">Calendar</span>
          </h2>
          <button
            onClick={() => {
              setIsSyncing(true);
          ...
          >
            <RefreshCw
              size={12}
              className={isSyncing ? "animate-spin text-orange-600" : ""}
            />
        ...
        <div className="flex gap-2 w-full lg:w-auto">
          <button
            onClick={exportToCSV}
            className="flex-1 lg:flex-none px-5 py-2.5 bg-zinc-900 border border-zinc-800 rounded-xl text-[9px] font-black uppercase flex items-center justify-center gap-2"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard label="Nodes" value={events.length.toString()} change="Org" icon={<TrendingUp size={14} />} />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
            ...
            <div className="w-full">
              <div className="grid grid-cols-7 gap-1 md:gap-2">
                {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
                  <div
                    key={day}
                    className="text-center text-[9px] font-black uppercase text-zinc-600 pb-2"
                  >
                    {day}
                  </div>
                ))}
                ...
                {Array.from({ length: 35 }).map((_, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className={`min-h-[70px] md:min-h-[90px] rounded-lg border p-1.5 transition-all group relative
                    ...
                    >
                      {isValidDay && (
                        <>
                          <span
                            className={`text-[10px] font-black ${isToday ? "text-orange-500" : "text-zinc-500"} group-hover:text-white`}
                          >
                            {dayNum}
                          </span>
                          <div className="mt-1 space-y-1">
                            {dayEvents.slice(0, 2).map((event) => (
                              ...
                              >
                                <p className="text-[7px] font-black uppercase truncate text-white">
                                  {event.title}
                                </p>
                              </button>
                            ))}
                            {dayEvents.length > 2 && (
                              <p className="text-[6px] font-bold text-zinc-600 text-center">
        ...
        <div className="xl:col-span-4 space-y-4">
          <div className="bg-zinc-900/20 border border-zinc-800/50 rounded-[1.5rem] p-5">
            <h3 className="text-[9px] font-black uppercase tracking-widest text-zinc-500 mb-4">
              Service Health
            </h3>
  ...
  return (
    <div className="bg-zinc-900/30 border border-zinc-800/50 p-4 rounded-2xl hover:bg-zinc-800/40 transition-all">
      <div className="flex justify-between items-center mb-2">
        ...
        <span className="text-[7px] font-black px-1.5 py-0.5 bg-emerald-500/10 text-emerald-500 rounded border border-emerald-500/20">
          {change}
        </span>
      </div>
...
function CategoryRow({ label, percentage, color }: { label: string; percentage: string; color: string }) {
  return (
