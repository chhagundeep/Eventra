...
interface DeleteModalProps {
  ...
  orgName: string;
  loading?: boolean;
  isRestricted?: boolean; // New prop to handle blocked deletions
}
...
export default function DeleteModal({
  ...
  orgName,
  loading,
  isRestricted = false, // Defaults to false
}: DeleteModalProps) {
  if (!isOpen) return null;

  // Handle clicking the background overlay to close
  const handleOverlayClick = (e: React.MouseEvent) => {
    // If it's loading or restricted, we might want to prevent accidental background closes, 
    // but usually, letting them close the modal to get back to work is best.
    ...
    >
      <div className="bg-[#111111] border border-white/10 p-6 rounded-2xl max-w-sm w-full shadow-2xl transform transition-all scale-100">
        ...
        <div className="flex gap-3 justify-end">
          {/* Main Action Button (Cancel/Back) */}
          <button
            onClick={onClose}
            disabled={loading}
          ...
          >
            {isRestricted ? "Understand" : "Cancel"}
          </button>

          {/* Conditional Rendering: 
            Only show the Terminate button if the deletion is NOT restricted 
          */}
          {!isRestricted && (
            <button
              onClick={onConfirm}
